...
- Candle materialization is currently handled by the separate [`cmd/candles`](../reference/code/cmd/candles/main) bridge (table name defaults to `candles`), while the `ohlcv_*` schemas in `ops/clickhouse/` represent the target analytics schema family.
