...
- **Purpose:** Efficiently insert `Trade` and `Candle` rows into ClickHouse via native protocol (`ch-go`) using columnar batching.
- **Primary knobs:** `BatchSize` and `FlushInterval` (service-level), plus DSN/table names.
- **Main risk area:** schema drift—this writer’s column mapping must stay aligned with `ops/clickhouse/*.sql`.
- **Candle note:** the `Candle` row type used by `cmd/candles` is not the same as the `ohlcv_*` analytics schema family; verify table compatibility in your deployment.
