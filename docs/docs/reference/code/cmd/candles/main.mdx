...
`cmd/candles/main.go` is a long-running **candle bridge** process:
...
Note: the ClickHouse destination table is configured via `CLICKHOUSE_CANDLES_TABLE` / `--clickhouse-candles` (default `"candles"`). The `ops/clickhouse/ohlcv_*` schemas are a separate (more canonical) candle schema family; this bridge currently writes the simplified `sinks/clickhouse.Candle` row type.
...
**Why:** The bridge’s job is glue: read from JetStream, decode protobuf, translate to sink row formats, write and ACK.
...
**What:** Configures and runs the candle bridge until shutdown.
...
**Why:** The canonical event stream remains integer-based for determinism; this bridge converts into the numeric types expected by the current ClickHouse candle writer.
...
**Why:** The Parquet/ClickHouse “canonical” candle schemas represent volumes as integers/decimals for exactness; this bridge converts into the float-based row type used by `sinks/clickhouse.Candle`.
