...
- starts the Go candle bridge (`cmd/candles`) to read candle subjects and write to ClickHouse
- replays a CSV of trade events using `candle_replay`
- stops the bridge and prints how many candle rows were inserted
...
- Go “bridge” consumes and writes to sinks
...
### Run Go candle bridge + replay trades

```bash title="Run Go candle bridge + replay trades" file=<rootDir>/scripts/run_candle_e2e.sh#L90-L112 showLineNumbers
```

**What:** Starts the Go bridge and replays trades through the C++ engine publisher.

**How:**

- Runs `go run ./cmd/candles` in the background consuming `${SUBJECT_ROOT}.candle.>`
- Uses `timeout` to bound the replay run
- Stops the bridge after a short settle period
