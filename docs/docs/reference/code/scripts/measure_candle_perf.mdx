...
- builds the C++ candle engine and the Go `cmd/candles` consumer/bridge
...
- builds the Go `cmd/candles` binary as `bridge-perf`
...
### Run bridge + replay trades

```bash title="Run bridge + replay trades" file=<rootDir>/scripts/measure_candle_perf.sh#L84-L113 showLineNumbers
```

**What:** Runs the Go candle consumer and replays the generated trades through the C++ publisher.
